...
def main():
    ...
    try:
        ...
        app_map = {g["appid"]: g.get("name") for g in games if "appid" in g}
        ...
        for target_appids in itertools.batched(list(app_map.keys()), batch_size):
            ...
            for appid, info in batch_results.items():
                ...
                steam_official_name = info.get("name")

                # If the game name has been set, use the set name
                config_name = app_map.get(appid)
                display_name = config_name if config_name else steam_official_name
                ...
                if last_build_id is None:
                    ...
                    logger.info(
                        f"First run for {display_name}. Saving BuildID: {current_build_id}")
                    state[appid] = current_build_id
                    state_changed = True
                elif current_build_id != last_build_id:
                    # Find different versions
                    logger.info(
                        f"Update detected for {display_name}! {last_build_id} -> {current_build_id}")
                    state[appid] = current_build_id
                    state_changed = True

                    # Send notification
                    notifier.notify(display_name, appid, last_build_id,
                                    current_build_id)
                else:
                    logger.info(
                        f"No update for {display_name}. (Current: {current_build_id})")
